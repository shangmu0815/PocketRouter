 ...
 *      DEFINES
 *********************/
#define WIN_HEADER_HOR_PR 50
#define WIN_PAGE_HOR_PR   140
...
lv_obj_t * lv_win_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    ext->btn_size = HEADER_ICON_SIZE;// [ZX53] Set info page header icon size
    ext->btnm = NULL;// [ZX53] Set custom btn matrix
    ...
    if(copy == NULL) {
        ...
        if(th) {
            ...
            lv_win_set_style(new_win, LV_WIN_STYLE_CONTENT, &lv_style_plain);
...
// [ZX53] Create a window objects that include button matrix
/**
 * Create a window objects that include button matrix
 * @param par pointer to an object, it will be the parent of the new window
 * @param btnm_map btn matrix to add in the bottom of win
 * @param btnm_action add btn matrix action
 * @return pointer to the created window
 */
lv_obj_t * lv_win_btn_create(lv_obj_t * par, const char ** btnm_map, lv_event_cb_t btnm_action) {
    return lv_win_btn_create_impl(par, btnm_map, btnm_action, NULL);
}

lv_obj_t * lv_win_btn_create_impl(lv_obj_t * par, const char ** btnm_map, lv_event_cb_t btnm_action,
        lv_event_cb_t btnm_action2) {
    LV_LOG_TRACE("window create started");

    /*Create the ancestor object*/
    lv_obj_t * new_win = lv_obj_create(par, NULL);
    lv_mem_assert(new_win);
    if(new_win == NULL) return NULL;
    ...
    lv_obj_set_size(new_win, LV_HOR_RES_MAX, LV_VER_RES_MAX);
    lv_obj_set_pos(new_win, 0, 0);
    lv_obj_set_style(new_win, &lv_style_pretty);
    ...
    ext->btnm = lv_btnm_create(new_win, NULL);
    lv_obj_set_protect(ext->btnm, LV_PROTECT_PARENT);
    lv_obj_set_parent(ext->btnm, new_win);

    //check if need reverse order for rtl
    set_reverse_btnm_map(ext->btnm, btnm_map);

    //if more than one btn, draw line between them
    static lv_style_t style_line;
    lv_btnm_ext_t * ext_btnm = lv_obj_get_ext_attr(ext->btnm);
    ...
    if (btn_cnt == 2) {
        ...
        lv_obj_t * line = lv_line_create(ext->btnm, NULL);
        lv_line_set_points(line, p, 2);
        lv_line_set_style(line, LV_LINE_STYLE_MAIN, &style_line);
    }

    if (btn_cnt == 3) {
        static lv_point_t p1[] = {{LV_HOR_RES_MAX/3, 10}, {LV_HOR_RES_MAX/3, 40}};
        static lv_point_t p2[] = {{LV_HOR_RES_MAX/3*2, 10}, {LV_HOR_RES_MAX/3*2, 40}};

        lv_obj_t * line1 = lv_line_create(ext->btnm, NULL);
        lv_line_set_points(line1, p1, 2);
        lv_line_set_style(line1, LV_LINE_STYLE_MAIN, &style_line);

        lv_obj_t * line2 = lv_line_create(ext->btnm, line1);
        lv_line_set_points(line2, p2, 2);

    }

    if (btnm_action2 != NULL) {
        lv_btnm_set_action2(ext->btnm, btnm_action2);
    } else {
        lv_obj_set_event_cb(ext->btnm, btnm_action);
    }
...
// [ZX53] Create a window objects that include button matrix end
/**
...
static void lv_win_realign(lv_obj_t * win)
{
    ...
    // [ZX53] Set win header height
    lv_obj_set_size(ext->header, lv_obj_get_width(win), WIN_HEADER_HOR_PR);
    // [ZX53] Set win header height end
    ...
    while(btn != NULL) {
        ...
        if(first_btn) {
            ...
            // [ZX53] Set second header icon to the left
            lv_obj_align(btn, ext->header, LV_ALIGN_IN_RIGHT_MID, -275 * LV_RES_OFFSET, 0);
        }
        btn_prev = btn;
        btn      = lv_obj_get_child_back(ext->header, btn);
    }
    ...
    // [ZX53] Set window header title to middle
    lv_obj_align(ext->title, NULL, LV_ALIGN_CENTER, style_header->body.padding.left, 0);
    ...
    // [ZX53] adjust obj if win include btn matrix
    if (ext->btnm) {
        lv_obj_set_size(ext->page, lv_obj_get_width(win), WIN_PAGE_HOR_PR * LV_RES_OFFSET);
        lv_obj_set_size(ext->btnm, lv_obj_get_width(win), WIN_HEADER_HOR_PR * LV_RES_OFFSET);
        lv_obj_align(ext->btnm, NULL, LV_ALIGN_IN_BOTTOM_LEFT, 0, 0);

    } else {
        lv_obj_set_size(ext->page, lv_obj_get_width(win), lv_obj_get_height(win) - lv_obj_get_height(ext->header));
    }
    // [ZX53] adjust obj if win include btn matrix end
}
