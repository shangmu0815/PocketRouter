...
#endif
// [ZX53] Adding customized page for keyboard to use
static lv_res_t lv_page_cust_scrollable_signal(lv_obj_t * scrl, lv_signal_t sign, void * param);
// [ZX53] Adding customized page for keyboard to use end
// [ZX53] Adding to draw sb background
static lv_area_t sb_bg_area;
static lv_style_t style_sb_bg;
// [ZX53] Adding to draw sb background end
...
lv_obj_t * lv_page_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    // [ZX53] Adding to draw sb background
    lv_style_copy(&style_sb_bg, &lv_style_plain);
    style_sb_bg.body.main_color = LV_COLOR_NOBEL;
    style_sb_bg.body.grad_color = LV_COLOR_NOBEL;
    // [ZX53] Adding to draw sb background end
...
// [ZX53] Adding customized page for keyboard to use
lv_obj_t * cust_lv_page_create(lv_obj_t * par, const lv_obj_t * copy)
{
    LV_LOG_TRACE("page create started");
    ...
    if(copy == NULL) {
        ext->scrl = lv_cont_create(new_page, NULL);
        lv_obj_set_signal_cb(ext->scrl, lv_page_cust_scrollable_signal);
        lv_obj_set_design_cb(ext->scrl, lv_scrl_design);
        lv_obj_set_drag(ext->scrl, true);
        lv_obj_set_drag_throw(ext->scrl, true);
        lv_obj_set_protect(ext->scrl, LV_PROTECT_PARENT | LV_PROTECT_PRESS_LOST);
        ...
        //lv_page_set_sb_mode(new_page, ext->sb.mode);
        lv_page_set_style(new_page, LV_PAGE_STYLE_BG, &lv_style_transp);
        lv_page_set_style(new_page, LV_PAGE_STYLE_SCRL, &lv_style_transp);
...
static bool lv_page_design(lv_obj_t * page, const lv_area_t * mask, lv_design_mode_t mode)
{
    ...
    if(mode == LV_DESIGN_DRAW_MAIN) {
        ...
        if(ext->sb.ver_draw && (ext->sb.mode & LV_SB_MODE_HIDE) == 0) {
            ...
            // [ZX53] Adding to draw sb background
            //only draw the sb background once

            /* refresh sb back ground by checking if gap's too big
             * issue happened if enter a page with sb bg and
             * enter another page without closing the previous page
             */
            int delta = abs(sb_bg_area.x2 - page->coords.x2);
            int padding = ext->sb.style->body.padding.inner + ext->sb.style->body.padding.top;
            if(ext->sb.ver_draw_bg || delta > padding) {
                ext->sb.ver_draw_bg = 0;
                lv_area_copy(&sb_bg_area, &sb_area);

                sb_bg_area.y1 = page->coords.y1 + padding/2;
                sb_bg_area.y2 = page->coords.y2 - padding/2;
            }
            lv_draw_rect(&sb_bg_area, mask, &style_sb_bg, lv_obj_get_opa_scale(page));
            // [ZX53] Adding to draw sb background end
