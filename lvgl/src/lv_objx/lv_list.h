...
typedef struct
{
    ...
    const lv_style_t *style_img2;                      /*Style of the list element images2 on buttons for ZX53*/
...
enum lv_list_selector_type {
    SELECTOR_TYPE_DEFAULT,
    SELECTOR_TYPE_CB,
    SELECTOR_TYPE_SW,
    SELECTOR_TYPE_COMBO_TYPE1,
    SELECTOR_TYPE_COMBO_TYPE2,
};

enum lv_list_selector_position { //check_box_location
    NONE,
    LEFT,
    RIGHT,
};
...
typedef struct {
    enum lv_list_selector_type type;
    enum lv_list_selector_position pos;
    bool enabled;
    lv_event_cb_t selector_release_action;
    lv_event_cb_t state_updator;
    char * metadata;
    lv_event_cb_t long_press_list_release_action;
} lv_list_selector_t;
 ...
 */
lv_obj_t * lv_list_add_for_PR(lv_obj_t * list, const void * img_src, const void * img_src2, const char * txt,
           lv_event_cb_t list_release_action, lv_list_selector_t selector_info, int item_id);

// [ZX53] Add cb and sw support in lv_list
lv_obj_t* list_content_action(lv_obj_t * list, const void * img_src, const void * img_src2, const char * txt,
                       lv_event_cb_t list_release_action, lv_list_selector_t selector_info, int item_id);

lv_obj_t * lv_list_add_for_combo(lv_obj_t * list, const void * img_src, const void * img_src2, const char * txt,
           lv_event_cb_t list_release_action, lv_list_selector_t selector_info, int item_id);

// [ZX53] Add list element defined size
#define LISTE_X (320 * LV_RES_OFFSET)
#define LISTE_Y (60 * LV_RES_OFFSET)
