...
// [ZX53] Add customized tabview add for dashboard
static lv_res_t tabpage_scrl_cust_signal(lv_obj_t * tab_scrl, lv_signal_t sign, void * param);
...
// [ZX53] Add customized tabview add for dashboard
static void tabpage_pressing_cust_handler(lv_obj_t * tabview, lv_obj_t * tabpage);
static void tabpage_press_lost_handler(lv_obj_t * tabview, lv_obj_t * tabpage);
// [ZX53] Add customized press lost handler add for launcher
static void tabpage_press_lost_cust_handler(lv_obj_t * tabview, lv_obj_t * tabpage);
...
// [ZX53] Add customized tabview add for dashboard
lv_obj_t * lv_tabview_cust_add_tab(lv_obj_t * tabview, const char * name)
{
    lv_tabview_ext_t * ext = lv_obj_get_ext_attr(tabview);
    ...
    lv_obj_t * h = lv_page_create(ext->content, NULL);
    lv_obj_set_size(h, lv_obj_get_width(tabview), lv_obj_get_height(ext->content));
    // [ZX53] do not show scroll bar in dashboard
    lv_page_set_sb_mode(h, LV_SB_MODE_OFF);
    // [ZX53] do not show scroll bar in dashboard end
    lv_page_set_style(h, LV_PAGE_STYLE_BG, &lv_style_transp);
    lv_page_set_style(h, LV_PAGE_STYLE_SCRL, &lv_style_transp);
    ...
    if(page_scrl_signal == NULL) page_scrl_signal = lv_obj_get_signal_cb(lv_page_get_scrl(h));
    lv_obj_set_signal_cb(h, tabpage_signal);
    lv_obj_set_signal_cb(lv_page_get_scrl(h), tabpage_scrl_cust_signal);
    ...
    /*Set the first btn as active*/
    // [ZX53] we only change this part for dashboard sliding issue
    if(ext->tab_cnt == 2) {
        ext->tab_cur = 1;
    }
    // [ZX53] we only change this part for dashboard sliding issue
...
// [ZX53] Add customized tabview add for dashboard end
...
static lv_res_t tabpage_scrl_signal(lv_obj_t * tab_scrl, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_PRESSED) {
        ...
        //[ZX53] customized press lost handler for launcher to use start
        tabpage_press_lost_cust_handler(tabview, tab_page);
        //[ZX53] customized press lost handler for launcher to use end
    }
...
static lv_res_t tabpage_scrl_cust_signal(lv_obj_t * tab_scrl, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_PRESSED) {
        ...
        tabpage_pressing_cust_handler(tabview, tab_page);
    } else if(sign == LV_SIGNAL_RELEASED || sign == LV_SIGNAL_PRESS_LOST) {
        tabpage_press_lost_handler(tabview, tab_page);
    }
...
static void tabpage_press_lost_cust_handler(lv_obj_t * tabview, lv_obj_t * tabpage)
{
   ...
   //[ZX53] implement for DLINK launcher tab slide circle start
#ifdef CUST_DLINK
    int end_tab = ext->tab_cnt - 1;
    if((id_prev == end_tab) && (page_x1 < 0) && (page_x2 < treshold)){
        //slide right
        lv_tabview_set_tab_act(tabview, 0, LV_ANIM_ON);
    }else if((id_prev == 0) && (page_x1 > 0) && (page_x1 > treshold)){
        //slide left
        lv_tabview_set_tab_act(tabview, end_tab, LV_ANIM_ON);
    }
    uint32_t dlink_id_new = lv_tabview_get_tab_act(tabview);
    res = lv_event_send(tabview, LV_EVENT_VALUE_CHANGED, &dlink_id_new);
#endif
    //[ZX53] implement for DLINK launcher tab slide circle end
