...
// [ZX53] Feedback button id in action
void lv_btnm_set_action2(lv_obj_t * btnm, lv_event_cb_t action)
{
    lv_btnm_ext_t * ext = lv_obj_get_ext_attr(btnm);
    ext->action2 = action;
}
// [ZX53] Feedback button id in action end
...
// [ZX53] Feedback button id in action
lv_btnm_ctrl_t lv_btnm_get_action2(const lv_obj_t * btnm)
{
    lv_btnm_ext_t * ext = lv_obj_get_ext_attr(btnm);
    return ext->action2;
}
...
static bool lv_btnm_design(lv_obj_t * btnm, const lv_area_t * mask, lv_design_mode_t mode)
{
    ...
    /*Draw the object*/
        ...
        for(btn_i = 0; btn_i < ext->btn_cnt; btn_i++, txt_i++) {
...
#endif
            lv_point_t txt_size;
            lv_txt_get_size(&txt_size, txt, font, btn_style->text.letter_space,
            ...
            lv_draw_label(&area_tmp, mask, btn_style, opa_scale, txt, txt_flag, NULL, -1, -1, NULL);
        }
