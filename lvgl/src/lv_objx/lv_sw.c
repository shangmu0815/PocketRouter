...
lv_obj_t * lv_sw_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    //[ZX53] fix switch btn on/off incorrect issue
    ext->init_value = -1;
    //[ZX53] fix switch btn on/off incorrect issue end
    ...
    if(copy == NULL) {
        //[ZX53] fixed switch size for Packet Router
        //lv_obj_set_size(new_sw, 2 * LV_DPI / 3, LV_DPI / 3);
        lv_obj_set_size(new_sw, LV_SW_PR_X, LV_SW_PR_Y);
        //[ZX53] fixed switch size for Packet Router end
...
static lv_res_t lv_sw_signal(lv_obj_t * sw, lv_signal_t sign, void * param)
{
    ...
    //[ZX53] fix switch btn on/off incorrect issue
    if(ext->init_value < 0){
        ext->init_value = lv_slider_get_value(sw);
    }
    //[ZX53] fix switch btn on/off incorrect issue end
