...
//[ZX53] ligature pairing map for Arabic Persian, order is first, second, ligature letter
const ap_liga_map_t ap_liga_table[] = {
    {{0xFEDF, 0xFE82}, 0xFEF5},  //ﻵ
    {{0xFEE0, 0xFE82}, 0xFEF6},  //ﻶ
    {{0xFEDF, 0xFE84}, 0xFEF7},  //ﻷ
    {{0xFEE0, 0xFE84}, 0xFEF8},  //ﻸ
    {{0xFEDF, 0xFE88}, 0xFEF9},  //ﻹ
    {{0xFEE0, 0xFE88}, 0xFEFA},  //ﻺ
    {{0xFEDF, 0xFE8E}, 0xFEFB},  //ﻻ
    {{0xFEE0, 0xFE8E}, 0xFEFC},  //ﻼ
};
/**********************
*      MACROS
**********************/
...
void _lv_txt_ap_proc(const char * txt, char * txt_out)
{
    ...
    //[ZX53] Do ligature for Arabic Persian start
    i = 0;
    j = 0;
    uint32_t table_length = sizeof(ap_liga_table)/sizeof((ap_liga_table)[0]);
    while(i < txt_length) {
        for(j = 0; j < table_length; j++) {
            if(ap_liga_table[j].ap_chars.first == ch_enc[i] && ap_liga_table[j].ap_chars.second == ch_enc[i + 1]){
                ch_enc[i] = 0xFEFF;
                ch_enc[i + 1] = ap_liga_table[j].ap_char_liga;
            }
        }
        i++;
    }
    //[ZX53] Do ligature for Arabic Persian end
