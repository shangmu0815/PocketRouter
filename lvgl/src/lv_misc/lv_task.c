...
static bool lv_task_debug = false;
...
LV_ATTRIBUTE_TASK_HANDLER void lv_task_handler(void)
{
    ...
    if(task_handler_mutex) {
        if(lv_task_debug) log_d("task_handler_mutex locked");
        return;
    }
    ...
    if(lv_task_run == false) {
        ...
        if(lv_task_debug) log_d("lv_task_run disabled");
        return;
    }
...
/**
 * Enable or disable lv_task debug log
 * @param en: true: lv_task debug is running, false: lv_task debug is suspended
 */
void lv_task_debug_enable(bool en)
{
    lv_task_debug = en;
}

/**
 * Dump lv_tasks debug info to log.
 */
void lv_task_dump(void)
{
    lv_task_t * next;
    ...
    while(LV_GC_ROOT(_lv_task_act)) {
        ...
        lv_task_t * task = LV_GC_ROOT(_lv_task_act);
        log_d("  task_cb: %p, period: %d, last_run: %d, elp: %d, prio: %d, once: %d",
               task->task_cb, task->period, task->last_run, lv_tick_elaps(task->last_run),
               task->prio, task->once);
