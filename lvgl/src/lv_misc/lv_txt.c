...
#include "lv_txt.h"
#include "lv_math.h"
// [ZX53] get current locale to fix character got cut short issue
#include "lv_pocket_router/src/util/util.h"
// [ZX53] get current locale to fix character got cut short issue end
/*********************
 *      DEFINES
 *********************/
        ...
        if(letter == '\n' || letter == '\r') {
            ...
            if(cur_w > max_width) {
                ...
                if(last_break != NO_BREAK_FOUND) {
                    ...
                    // [ZX53] fix character got cut short issue, we use the one before previous char
                    if(is_txt_CJK(txt) || is_txt_rtl(txt)){
                        i = i - 3;
                    }
                    // [ZX53] fix character got cut short issue, we use the one before previous char end
                }
